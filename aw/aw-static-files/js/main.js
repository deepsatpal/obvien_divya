...
var common_elements_among_search_results = {
  edu: { __max__: { edu: "", count: 0 } },
  job: { __max__: { org: "", count: 0 } },
};
...
function viewSaveSearch(data) {
  ...
  if (seachFilters != "null") {
    ...
    for (i = 0; i < Object.keys(filters).length; i++) {
      ...
      field_placeholders[filter_name] = $(`input[name="${filter_name}"]`).attr(
        "placeholder"
      );
    }
  }
  ...
  for (let filter_key in search_view_obj["filters"]) {
    ...
    if (filter_key == "job_title") {
      document.getElementById("job_title").value =
        search_view_obj["filters"][filter_key];
    }
    ...
    if (filter_key == "sub_industry") {
      document.getElementById("sub_industry").value =
        search_view_obj["filters"][filter_key];
    }
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        data.forEach((job_edu_profile) => {
          if ("job_profile" in job_edu_profile) {
            if (job_edu_profile["job_profile"][0]["organization_"] !== null) {
              job_profile_org_suggest_list.push(
                job_edu_profile["job_profile"][0]["organization_"]
              );
            }
            if (
              job_edu_profile["job_profile"][0]["organization_title_"] !== null
            ) {
              job_profile_tilte_suggest_list.push(
                job_edu_profile["job_profile"][0]["organization_title_"]
              );
            }
          }
          if ("edu_profile" in job_edu_profile) {
            if (job_edu_profile["edu_profile"][0]["school_"] !== null) {
              edu_profile_school_suggest_list.push(
                job_edu_profile["edu_profile"][0]["school_"]
              );
            }
            if (job_edu_profile["edu_profile"][0]["degree_"] !== null) {
              edu_profile_degree_suggest_list.push(
                job_edu_profile["edu_profile"][0]["degree_"]
              );
            }
          }
        });
        ...
        $("#organization_name").easyAutocomplete({
          data: job_profile_org_suggest_list
          ...
          list: {
            ...
            onSelectItemEvent: function () {
              change_field_value_on_selection(
                "organization_name",
                "onSelectItemEvent"
              );
            },
        ...
        $("#school_name").easyAutocomplete({
          data: edu_profile_school_suggest_list
            .filter((value, index, self) => self.indexOf(value) === index)
            .sort(sortFilterList),
          list: {
        ...
        $("#degree").easyAutocomplete({
          data: edu_profile_degree_suggest_list
            .filter((value, index, self) => self.indexOf(value) === index)
            .sort(sortFilterList),
          list: {
        ...
        $("#job_title").easyAutocomplete({
          data: job_profile_tilte_suggest_list
            .filter((value, index, self) => self.indexOf(value) === index)
            .sort(sortFilterList),
          list: {
...
*/
          ...
          if (file_tags_suggestions_count > 0) {
            ...
            if (file_tags_checkbox_list !== "") {
              if (
                typeof file_tags_checkbox_list == "object" &&
                file_tags_checkbox_list.length <
                  data.file_tags_suggestions.length
              ) {
            ...
            for (let ftl = 0; ftl < file_tags_suggestions_count; ftl++) {
              ...
              if (file_tags_checkbox_list !== "") {
                if (typeof file_tags_checkbox_list == "object") {
                  if (
                    file_tags_checkbox_list.indexOf(
                      data.file_tags_suggestions[ftl]
                    ) == -1
                  ) {
                    ind_filetag_checked = "";
                  }
          ...
          function extract_country_cities(event_type = "") {
            ...
            let city_options = {
              data: country_cities_list.filter(
                (value, index, self) => self.indexOf(value) === index
              ),
              getValue: "name",
              adjustWidth: false,
          ...
          let country_options = {
            data: org_countries_list.filter(
              (value, index, self) => self.indexOf(value) === index
            ),
            //                        getValue: "name",
            adjustWidth: false,
          ...
          $("#search_resut_count").text(
            total_results +
              " results found for " +
              '"' +
              search_obj["search_str"] +
              '"'
          );

          if (
            "view_saved_search" in data &&
            data["view_saved_search"] == true
          ) {
            $.each(data.filter_weights, function (filter_field, filter_weight) {
              $(`#${filter_field}`)
                .bootstrapSlider("enable")
                .bootstrapSlider("setValue", filter_weight);
            });
          }
          ...
          $(".aw_wrapper").preloader("remove");
          var html =
            '<div class="not-found-results"><div class="not-found-placeholder">No result found</div></div>';
  ...
  $(".filter").change(function (e) {
    ...
    $(`input[group="${filter_group}"]`).each(function () {
      ...
      if (name && val.trim()) {
        ...
        field_placeholders[name] = $(`input[name="${name}"]`).attr(
          "placeholder"
        );
...
*/
  ...
  $(document).on("click", ".fa-times", function () {
    ...
    $(`#${filter_name}_weightage`)
      .bootstrapSlider("setValue", 0)
      .bootstrapSlider("disable");

    if (
      filter_name == "member_of_platform" ||
      filter_name == "degree_of_connection"
    ) {
  ...
  function result_card(
    ...
    let result_job_location = concat_loc(
      result_obj.organization_city_1,
      result_obj.organization_country_1
    );
    let result_organization_title = empty_if_invalid_val(
      result_obj.organization_title_1
    );
    ...
    while (ico_count <= result_obj["org_job_to_from_count"]) {
      if (
        listed_platforms.indexOf(result_obj["job_platform_" + ico_count]) == -1
      ) {
        ...
        if (result_obj["job_platform_" + ico_count]) {
          ...
          if (
            result_obj["job_platform_" + ico_count].toLowerCase() == "csv" ||
            result_obj["job_platform_" + ico_count].toLowerCase() == "null"
          ) {
            ico_count++;
            continue;
          }
    ...
    if (search_feedback.length > 0) {
      if (
        (contact_index = search_feedback.findIndex(
          (contact_feeback) => contact_feeback.contact == result_obj.contact_id
        )) > -1
      ) {
...
<div class="aw_card_likes">
  ...
  function swiper_slider(
    ...
    if (first_degree_relation) {
      var containsFirstDegree =
        first_degree_relation.indexOf(degree_contact_id) > -1;
    }
    if (second_degree_relation) {
      var containsSecondDegree =
        second_degree_relation.indexOf(degree_contact_id) > -1;

      let common_features_org_count =
        data["common_features"]["organization"].length;
      let common_features_edu_count =
        data["common_features"]["education"].length;
      ...
      if (
        !containsFirstDegree &&
        containsSecondDegree &&
        degree_contact_id in response.mutual_connections
      ) {
        for (
          let fdci = 0;
          fdci < response.mutual_connections[degree_contact_id].length;
          fdci++
        ) {
          mutual_connections_list.push({
            name: response.first_degree_connections_details[
              response.mutual_connections[degree_contact_id][fdci]
            ]["full_name"],
            photo: data["photo"],
          });
        }
        ...
        if (number_of_mutual_connections > 0) {
          ...
          for (let mii = 0; mii < number_of_mutual_connections; mii++) {
            ...
            if (
              mutual_connections_list[mii]["photo"] != "" &&
              mutual_connections_list[mii]["photo"] != "null" &&
              mutual_connections_list[mii]["photo"] != null &&
              mutual_connections_list[mii]["photo"] !== undefined &&
              mutual_connections_list[mii]["photo"] !== "undefined"
            ) {
...
${mutual_connections_names_html_string}
    ...
    if (third_degree_relation) {
      var containsThirdDegree =
        third_degree_relation.indexOf(degree_contact_id) > -1;
    }
    ...
    if (containsFirstDegree == true) {
      ...
      if (data["contact_id"] in response.connection_graph) {
        ...
        if ("sec_degree_connection_details" in response) {
          ...
          remote_sec_deg_connection_details =
            response.sec_degree_connection_details;
        }
        ...
        console.log(
          "Response Graph ",
          response.connection_graph[data["contact_id"]]
        );
      }
    } else {
    ...
    function* gen_des_content(data) {
      ...
      function des_not_shown(des_obj) {
        for (let des_index = 0; des_index < listed_des.length; des_index++) {
          if (
            des_obj["des"] == listed_des[des_index]["des"] &&
            des_obj["link"] == listed_des[des_index]["link"] &&
            des_obj["platform"] == listed_des[des_index]["platform"]
          ) {
            return false;
          }
        }
      ...
      while ("contact_description_" + counter in data && counter <= 20) {
        ...
        let des_counter = empty_if_invalid_val(
          data["contact_description_" + counter]
        );
        let des_info = {
          des: des_counter,
          des: des,
    ...
    function* gen_exp_content(data) {
      ...
      function experience_not_shown(exp_obj) {
        for (
          let exp_index = 0;
          exp_index < listed_experience.length;
          exp_index++
        ) {
          if (
            exp_obj["org"] == listed_experience[exp_index]["org"] &&
            exp_obj["org_title"] == listed_experience[exp_index]["org_title"] &&
            exp_obj["org_start"] == listed_experience[exp_index]["org_start"] &&
            exp_obj["org_end"] == listed_experience[exp_index]["org_end"] &&
            exp_obj["link"] == listed_experience[exp_index]["link"] &&
            exp_obj["platform"] == listed_experience[exp_index]["platform"]
          ) {
            return false;
          }
        }
      ...
      while ("organization_" + counter in data && counter <= 20) {
        let exp_info_ = "";
        let org_title = empty_if_invalid_val(
          data["organization_title_" + counter]
        );
        let org_start = empty_if_invalid_val(
          data["organization_start_" + counter]
        );
        let org_end = empty_if_invalid_val(data["organization_end_" + counter]);
        let org_city = empty_if_invalid_val(
          data["organization_city_" + counter]
        );
        let org_country = empty_if_invalid_val(
          data["organization_country_" + counter]
        );
        ...
        if (org_counter != "") {
          if (org_counter in common_elements_among_search_results["job"]) {
            ...
            if (
              common_elements_among_search_results["job"][org_counter] >
              common_elements_among_search_results["job"]["__max__"]["count"]
            ) {
              common_elements_among_search_results["job"]["__max__"] = {
                org: org_counter,
                count: common_elements_among_search_results["job"][org_counter],
              };
            }
          } else {
            common_elements_among_search_results["job"][org_counter] = 1;
          ...
          console.log(
            "common_elements_among_search_results ",
            common_elements_among_search_results
          );
        }
    ...
    function* gen_edu_content(data) {
      ...
      function education_not_shown(edu_obj) {
        for (
          let edu_index = 0;
          edu_index < listed_education.length;
          edu_index++
        ) {
          if (
            edu_obj["edu_degree"] ==
              listed_education[edu_index]["edu_degree"] &&
            edu_obj["edu_school"] ==
              listed_education[edu_index]["edu_school"] &&
            edu_obj["edu_start"] == listed_education[edu_index]["edu_start"] &&
            edu_obj["edu_end"] == listed_education[edu_index]["edu_end"] &&
            edu_obj["link"] == listed_education[edu_index]["link"] &&
            edu_obj["platform"] == listed_education[edu_index]["platform"]
          ) {
            return false;
          }
        }
      ...
      while ("school_" + counter in data && counter <= 20) {
        ...
        if (edu_school != "") {
          if (edu_school in common_elements_among_search_results["edu"]) {
            ...
            if (
              common_elements_among_search_results["edu"][edu_school] >
              common_elements_among_search_results["edu"]["__max__"]["count"]
            ) {
              common_elements_among_search_results["edu"]["__max__"] = {
                edu: edu_school,
                count: common_elements_among_search_results["edu"][edu_school],
              };
            }
          } else {
            common_elements_among_search_results["edu"][edu_school] = 1;
    ...
    var html =
      ...
      degree_relation +
      "</h3>" +
      '<div class="aw_card_location_swiper">' +
      location +
      "</div>" +
      '<div class="aw_card_subtitle">' +
      job_title +
      "</div>" +
      '<div class="aw_card_meta">' +
      company +
      "</div>" +
      '<div class="aw_card_meta">' +
      industry +
      ...
      '<div class="aw_card_reason"><p>' +
      third_and_third_plus_connections +
      "</p></div>" +
  ...
  function get_search_reason(data) {
    ...
    for (let ofi = 0; ofi < occr_fields_list.length; ofi++) {
      ...
      if (occr_field_name == "first_name" || occr_field_name == "last_name") {
        ...
        let name = "";
        if (
          data["highlights"].hasOwnProperty("first_name") ||
          data["highlights"].hasOwnProperty("last_name") ||
          data["highlights"].hasOwnProperty("full_name")
        ) {
...
*/
  ...
  function get_warmth_of_relationship(data) {
    ...
    let commonly_attended_institutions_types_len =
      commonly_attended_institutions_types.length;

    if (commonly_attended_institutions_types_len > 0) {
      num_of_common_institutions +=
        data["common_institutions"]["education"].length;
      num_of_common_institutions +=
        data["common_institutions"]["organization"].length;
      ...
      if (num_of_common_institutions > 0) {
        for (
          let cai_index = 0;
          cai_index < commonly_attended_institutions_types_len;
          cai_index++
        ) {
          if (
            common_institutions[commonly_attended_institutions_types[cai_index]]
              .length > 0
          ) {
            for (
              let ci = 0;
              ci <
              common_institutions[
                commonly_attended_institutions_types[cai_index]
              ].length;
              ci++
            ) {
              if (
                already_listed_common_institution.indexOf(
                  common_institutions[
                    commonly_attended_institutions_types[cai_index]
                  ][ci]
                ) === -1
              ) {
                already_listed_common_institution.push(
                  common_institutions[
                    commonly_attended_institutions_types[cai_index]
                  ][ci]
                );
  ...
  function concat_loc(city, country) {
    return empty_if_invalid_val(city)
      ? city + ", " + empty_if_invalid_val(country)
      : empty_if_invalid_val(country);
  }
      ...
      success: function (cities_list) {
        let city_options = {
          ...
          listLocation: function (country_states) {
            ...
            cities_suggestions = country_states.states.flatMap(
              (state) => state.cities
            );
  ...
  $("body").on("change", ".file-tags-suggestions", function () {
    ...
    if (typeof file_tags_checkbox_list == "object") {
      checkbox_filetag_index = file_tags_checkbox_list.indexOf(
        checkbox_filetag_value
      );
    }
    ...
    if (checkbox_filetag_value == "all") {
      ...
      if ($(this).prop("checked") === true) {
        ...
        if (
          file_tags_checkbox_list.length === file_tags_suggestions_list.length
        ) {
  ...
  $("body").on("click", ".third-degree-mut", function () {
    ...
    for (let pl = 0; pl < paths_list.length; pl++) {
      ...
      if (
        remote_sec_deg_connection_details[sdc]["job_title"] &&
        remote_sec_deg_connection_details[sdc]["organization_name"]
      ) {
...
<div class="mdl_conn_detail"><span class="sec_fst_deg_con_names">${first_degree_connection_names}${first_degree_name_separator}</div>
  ...
  $("body").on("click", ".tweet_notification_link", function () {
    ...
    let $tp_tweets_html = $(`#tp-tweets-${contact_id}`)
      .clone()
      .attr("id", `tp-tweets-modal-${contact_id}`)
      .removeClass("tp_detail_tweets");
