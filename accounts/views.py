...
def signup_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            activate_account(request , user.profile.email)
            
            # print("Activation User EMAIL ", emailAddress_add.useremail)
            # print("Activation User ID ",emailAddress_add.activation_id)
            ...
            for i in emailsplit:
                try:      
                    if len(User.objects.filter(email=i))==0:    
                        print('in if')             
                        send_email_invite(first_name, i)
                    else:
                        print('in else')
...
def send_activation_email_to_admin (user, activation_string  ):
    ...
    send_mail(
        ...
        ['ssingh@codenomad.net'],
        html_message=f'<p>A user just got registered. Please click <a href="http://altworkz.com/accounts/account-activation/{activation_string}">here</a> to activate this account.</p><p>Alternatively, you can also login into <a href="http://altworkz.com/admin/auth/user/{user.id}/change/">admin panel</a> and enable / disable account of any user you want by checking / unchecking Active.</p>',
    )
...
def activate_account (request  , email   ):
    ...
    send_mail(
        ...
        'altworkzmailmgr@rakbridges.com',
        [email],            
        html_message='Your access to Altworkz is approved by admin. You can visit <a href="http://altworkz.com/">Altworkz</a> to proceed',
    ) 
...
def send_email_invite(first_name , email   ):
        try:
            send_mail(
                    '',
                    '',
                    ...
                    html_message=f'''<p>Hi {email},
                        ...
                        <a style="padding:4px 6px;outline:1px solid #008;background-color:#fff" href='http://altworkz.com/accounts/signup/'>
                        Click Here </a>
